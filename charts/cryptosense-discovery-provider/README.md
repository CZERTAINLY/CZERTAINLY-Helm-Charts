# Cryptosense Discovery Provider - ILM
...
```bash
helm show values oci://hub.omnitrustregistry.com/ilm-helm/cryptosense-discovery-provider > values.yaml
```
...
```bash
helm install --namespace ilm -f values.yaml ilm-cryptosense-discovery-provider oci://hub.omnitrustregistry.com/ilm-helm/cryptosense-discovery-provider
```
...
```bash
helm upgrade --namespace ilm -f values.yaml ilm-cryptosense-discovery-provider oci://hub.omnitrustregistry.com/ilm-helm/cryptosense-discovery-provider
```
...
```bash
helm uninstall --namespace ilm ilm-cryptosense-discovery-provider
```
...
| Parameter                                    | Default value                               | Description                                                                                                            |
|----------------------------------------------|---------------------------------------------|------------------------------------------------------------------------------------------------------------------------|
| nameOverride                                 | `cryptosense-discovery-provider`            | Override for the chart name. Used as the `app.kubernetes.io/name` selector label value and as input to the fullname helper. Pinned to keep selectors stable across chart renames; changing this requires manual cleanup of existing Deployments. |
...
| image.name                                   | `cryptosense-discovery-provider` | Docker image name                                                                                                      |
...
| serviceAccount.name                          | `"cryptosense-discovery-provider-sa"`       | The name of the service account to use. If not set and create is true, a name is generated using the fullname template |
