...
curl -X PUT \
  ...
  -d '
  {
    ...
    "tokenUrl": "http://keycloak-internal-service.{{ .Release.Namespace }}.svc.{{ .Values.clustering.domain }}:{{ $internalKeycloakHttpPort }}/kc/realms/CZERTAINLY/protocol/openid-connect/token",
    "logoutUrl": "https://{{ $hostName }}/kc/realms/CZERTAINLY/protocol/openid-connect/logout",
    "jwkSetUrl": "http://keycloak-internal-service.{{ .Release.Namespace }}.svc.{{ .Values.clustering.domain }}:{{ $internalKeycloakHttpPort }}/kc/realms/CZERTAINLY/protocol/openid-connect/certs",
    "scope": ["openid"],
