# EJBCA Legacy Connector - CZERTAINLY
...
```bash
helm show values oci://harbor.3key.company/czertainly-helm/ejbca-legacy-connector > values.yaml
```
...
```bash
helm install --namespace czertainly -f values.yaml czertainly-ejbca-legacy-connector oci://harbor.3key.company/czertainly-helm/ejbca-legacy-connector
```
...
```bash
helm upgrade --namespace czertainly -f values.yaml czertainly-ejbca-legacy-connector oci://harbor.3key.company/czertainly-helm/ejbca-legacy-connector
```
...
```bash
helm uninstall --namespace czertainly czertainly-ejbca-legacy-connector
```
